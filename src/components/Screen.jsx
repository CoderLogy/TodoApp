...
const Screen = ({
  ...
  useEffect(() => {
    if (lastAddedId?.current && todoRefs.current[lastAddedId.current]) {
      const todoElement = todoRefs.current[lastAddedId.current];
      const todoText = todos.find((t) => t.id === lastAddedId.current)?.todo;

      if (todoText) {
        gsap.fromTo(
          todoElement,
          { scrambleText: { value: todoText, chars: "▓░█▓░" } },
          {
            duration: 1,
            scrambleText: todoText,
            ease: "power2.out",
          }
        );
  ...
  const handleDeleteWithAnimation = (id) => {
    const todoElement = todoRefs.current[id];
    if (todoElement) {
      gsap.to(todoElement, {
        duration: 1.2,
        scrambleText: "DELETED",
        ease: "power2.out",
        onComplete: () => {
          deleteTodo(id);
        },
  ...
  return (
    <div
      className="screen shadow-lg shadow-zinc-600 rounded-lg"
      style={{
          ...
          {todos.length > 0 && (
            <ReactSortable list={todos} setList={setTodos} animation={150}>
              {todos.map((todo) => (
                <div
                  key={todo.id}
                  className="todos flex  w-full text-md sm:text-xl my-7 justify-between"
                >
                  <label className="flex items-center flex-1 min-w-0 space-x-2 mx-7 lg:mx-6">
                    <input
                      ...
                      className="checkbox checkbox-md sm:checkbox-lg"
                    />
                    <span
                      ref={(el) => {
                        todoRefs.current[todo.id] = el;
                      }}
                      className={`mx-1.5 -mt-0.5 break-all ${todo.isCompleted ? "line-through text-gray-400" : ""}`}
                    >
                      {todo.todo}
                    </span>
                  </label>
                  <div className="flex gap-0 sm:gap-2">
                    <button
                      className="btn btn-sm sm:btn-md text-md flex-1 w-20"
                      onClick={() => {
                        editTodo(todo.id);
                      }}
                    >
                    ...
                    <button
                      className="btn btn-sm sm:btn-md text-md mx-3 flex-1"
                      onClick={() => {
                        handleDeleteWithAnimation(todo.id);
                      }}
                    >
                      Delete
                    </button>
